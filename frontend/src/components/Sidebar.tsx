        ...
        {modes.map(({ key, icon }) => (
          <button
            ...
            style={{
              ...
              background: workflowMode === key ? 'rgba(74, 222, 128, 0.1)' : 'transparent',
              borderColor: workflowMode === key ? 'var(--accent-primary)' : 'transparent',
              color: workflowMode === key ? 'var(--accent-primary)' : 'var(--text-primary)',
              fontWeight: workflowMode === key ? 600 : 400,
            }}
            onClick={() => setWorkflowMode(key)}
          >
            {icon} {WORKFLOW_LABELS[key]}
          </button>
        ))}
      </div>
